...
class StudyPlannerAgent:
    ...
    async def _planner_node(self, state: PlannerStateDict) -> dict:
        """Generate study plan using LLM. 3 retries, output as list of calendar events."""
        ...
        # Build free time description
        free_desc = self._build_free_desc(state)

        # Gather context chunks (from ingestor search)
        context_text = await self.qdrant_service.gather_context("", subject_name) if self.qdrant_service else ""

        # Build recommend section
        recommend = state.get("recommend_context", "")
        recommend_section = ""
        if recommend:
        ...
        for attempt in range(1, PLANNER_MAX_RETRIES + 1):
            try:
                ...
                if not text:
                    logger.warning(f"[planner] Attempt {attempt}: empty response")
                ...
                if plan:
                    ...
                    # Save plan to DB
                    self._save_plan_to_db(state, subject, plan)
                ...
                logger.error(f"[planner] Attempt {attempt} error: {e}")
    ...
    def _save_plan_to_db(self, state: PlannerStateDict, subject: Dict, plan: List[Dict]):
